...
function animalClassify(type, weight) {
    // Your implementation here
    if (weight < 5) {
        return {type: type, size: "small"};
    }
    if (weight >= 5 && weight < 100) {
        return {type: type, size: "big"};
    }
    if (weight >= 100) {
        return {type: type, size: "huge"};
    }
}
...
function describePerson(person) {
    ...
    // let hobby 
    // if ('hobby' in person) {
    //     hobby = `${pronoun} likes ${person.hobby}!`;
    // } else {
    //     hobby = `${pronoun} has no hobbies.`
    // }
    let pronoun = person.gender == 'm' ? "He" : "She";
    let hobby = 'hobby' in person ? `${pronoun} likes ${person.hobby}!` : `${pronoun} has no hobbies.`;

    // let manwoman = person.gender == 'm' ? 'man' : 'woman';
    // return "hello world";
    // return `${person.name} is a ${person.age}-year-old ${manwoman}.${hobby}`;
    return `${person.name} is a ${person.age}-year-old ${person.gender == 'm' ? 'man' : 'woman'}. ${hobby}`;
}
