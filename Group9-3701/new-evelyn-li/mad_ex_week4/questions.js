...
function createStudentFile(studentName, studentInfo, callback) {
  ...
  const { firstName, surName, age, hobby } = studentInfo;
  const fileName = studentName.toLowerCase().replace(" ", "") + ".txt";
  ...
  const fileContent = `Name: ${studentName}\nAge: ${age}\nHobby: ${hobby.join(
    ", "
  )}`;
  setTimeout(() => {
...
function checkState(board) {
  ...
  for (let i = 0; i < allLines.length; i++) {
    if (allLines[i].every((cell) => cell === "X")) {
      return "X wins";
    }
    if (allLines[i].every((cell) => cell === "O")) {
      return "O wins";
    }
  }
  ...
  if (board.every((cell) => cell !== "") && xCount === oCount + 1) {
    return "It is a tie";
  }
