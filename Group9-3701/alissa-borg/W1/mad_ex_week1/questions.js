...
function capitalizeSentence(sentence) {
  ...
  return capitalizedSentence.trim(); // Trim to remove trailing space
}
...
function describePerson(person) {
  ...
  let pronoun, hobbiesDescription;
  if (gender === "m") {
    pronoun = "He";
  } else {
    pronoun = "She";
  ...
  if (hobby) {
    hobbiesDescription = `${pronoun} likes ${hobby}!`;
  } else {
    hobbiesDescription = `${pronoun} has no hobbies.`;
  }
  return `${name} is a ${age}-year-old ${gender === "m" ? "man" : "woman"}. ${hobbiesDescription}`;
}
