...
function accountGenerator(accountName, initBalance) {
  ...
  return function(amount) {
    balance += amount;
    return `You deposited ${amount} to your Saving Account and the current balance is ${balance}.`;
  };
}
 ...
 */
 ...
 function distributeTips(...tips) {
  ...
  return { food: totalFood, drink: totalDrink };
}
...
function calculateAlternatingHarmonic(n, accumulator = 0, index = 1) {
  ...
  return calculateAlternatingHarmonic(
    n,
    index % 2 === 0 ? accumulator - 1 / index : accumulator + 1 / index,
    index + 1
  );
}
