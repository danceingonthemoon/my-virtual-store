...
function doubleDivisibleByThree(numbers) {
  // Your implementation here
  let trips = numbers.filter(number => number%3===0);
  return trips.map(trips => trips*2);
}
...
function selectHighPerformingStudents(students) {
  // Your implementation here
  let gpas = students.filter(student => student.GPA >= 5 && student.hobbies.includes('coding'));
  let arr = gpas.map(student => ({name: student.name, email: student.email}));
...
function aggregateStudentData(students) {
  ...
  const studentAvgGPA = Number((students.reduce((accumulate, student) => accumulate + student.GPA, 0) / studentNum));
...
function swapForm(input) {
  // Your implementation here
  if (input.includes(' ')) {
    return input.toLowerCase().split(' ').map((input, i) => {
      if(i===0) return input;
      return input.slice(0,1).toUpperCase()+input.slice(1)
    }).join('');
  } else {
    return input.split('').map(a=>a.toUpperCase()===a ? " "+a.toLowerCase():a).join('');
  }
