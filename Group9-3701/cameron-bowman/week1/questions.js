...
function greeting(name, time) {
  // Your implementation here
  const morn = 'Good morning, ' + name + '!';
  const aft = 'Good afternoon, ' + name + '!';
...
function describePerson(person) {
  // Your implementation here
  let heOrShe = person.gender == 'm' ? "He" : "She";
  let mOrF = person.gender == 'm' ? "man" : 'woman';
  let hobbies = '';

  if (person.hobby) {
    hobbies = `${heOrShe} likes ${person.hobby}!`;
  } else {
    hobbies = `${heOrShe} has no hobbies.`;
  }

  return `${person.name} is a ${person.age}-year-old ${mOrF}. ${hobbies}`;
